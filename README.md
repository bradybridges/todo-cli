...
A command line tool to manage a simple todo list, usable as either an interactive terminal UI or a scriptable CLI.
...
Run `todo` on its own for the interactive TUI, or pass it a subcommand for direct CLI usage.
...
- [CLI](#cli)
  ...
  - [Delete specific tasks](#delete-specific-tasks)
...
Tasks and settings are persisted between runs, so anything you add or configure in one mode (TUI or CLI) is immediately available in the other.
...
Task IDs are the numbers shown next to each task in `todo list`.

```
todo delete -t <taskId...>
todo delete -t 1 3 5
```
